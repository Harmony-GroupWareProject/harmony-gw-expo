...
const AddEventModal = ({ visible, onClose, onSave, selectedDate }) => {
  ...
  useEffect(() => {
    if (visible) {
      setNewEventTitle('');
      setNewEventStartDate(new Date(selectedDate));
      setNewEventStartTime(format(new Date(), 'HH:00'));
      setNewEventEndDate(new Date(selectedDate));
      setNewEventEndTime(format(new Date(), 'HH:00'));
      setIsAllDay(false);
